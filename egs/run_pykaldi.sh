...
../lattice-remove-ctc-blank.py 1 ark:input.txt ark,t:output.txt;

KALDI="docker-command-line-interface -- mauvilsa/kaldi:2018.08.27-ubuntu16.04";

# Plot original lattice in PDF
$KALDI lattice-to-fst --acoustic-scale=1 --lm-scale=1 ark:input.txt ark,t:- \
  | tail -n+2 \
  | $KALDI fstcompile \
  | $KALDI fstdraw --portrait --isymbols=symbs.txt --osymbols=symbs.txt \
  | dot -Tpdf > input.pdf

# Plot output lattice in PDF
$KALDI lattice-to-fst --acoustic-scale=1 --lm-scale=1 ark:output.txt ark,t:- \
  | tail -n+2 \
  | $KALDI fstcompile \
  | $KALDI fstdraw --portrait --isymbols=symbs.txt --osymbols=symbs.txt \
  | dot -Tpdf > output.pdf
...
# part of the cost.
$KALDI lattice-to-nbest --n=27 ark:input.txt ark:- \
  | $KALDI nbest-to-linear ark:- ark,t:- ark:/dev/null ark,t:- \
  | awk '{
      if (NF == 2) {
        COST[$1] = $2;
      } else {
...
# paths).
$KALDI lattice-to-nbest --n=27 ark:output.txt ark:- \
  | $KALDI nbest-to-linear ark:- ark,t:- ark:/dev/null ark,t:- \
  | awk '{
      if (NF == 2) {
        COST[$1] = $2;
...
$KALDI lattice-to-nbest --n=27 ark:output.txt ark:- \
  | $KALDI nbest-to-linear ark:- ark:/dev/null ark,t:- ark,t:- \
  | awk '{
      if (!($1 in PATH)) {
        PATH[$1] = $2;
        for(i=3;i<=NF;++i) {
